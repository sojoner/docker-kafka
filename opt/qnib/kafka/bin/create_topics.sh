...
ZK_HOST=${KAFKA_ZOOKEEPER_HOST:-zookeeper.service.consul}
...
function qnib_kafka_bootstrap_topics {
    for topic in $(echo ${1}|sed -e 's/,/ /g');do
        ...
        if [ $(/opt/kafka/bin/kafka-topics.sh --zookeeper ${ZK_HOST}:2181 --describe --topic ${name}|wc -l) -ne 0 ];then
	    echo "It seems that '${name}' already exists..."
            /opt/kafka/bin/kafka-topics.sh --zookeeper ${ZK_HOST}:2181 --describe --topic ${name}
        elif [ $(echo ${topic} | awk 'BEGIN{FS=":"} {print NF}') -eq 1 ];then
            /opt/kafka/bin/kafka-topics.sh --zookeeper ${ZK_HOST}:2181 --replication-factor 3 --partitions 1 --create --topic ${name}
        elif [ $(echo ${topic} | awk 'BEGIN{FS=":"} {print NF}') -eq 2 ];then
            replicas=$(echo ${topic}|awk -F\: '{print $2}')
            /opt/kafka/bin/kafka-topics.sh --zookeeper ${ZK_HOST}:2181 --replication-factor ${replicas} --partitions 1 --create --topic ${name}
